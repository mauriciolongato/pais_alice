...
def generate_mail_messages(gmail_user_name, p_word, resume_file):
    ...
    for msgId in data[0].split():
        ...
        if msgId not in ProcessedMsgIDs:
            yield email.message_from_bytes(email_body) # - versao para python3
            #yield email.message_from_string(email_body)
...
if __name__ == '__main__':
    workers_login = ["worker.tf.alice.1@gmail.com", "worker.tf.alice.2@gmail.com"]
    workers_password = ["terrafirma_worker_1", "terrafirma_worker_2"]
    for (user_name, password) in zip(workers_login, workers_password):
        resumeFile = os.path.join('resume.txt')
        user_name = user_name
        password = password
        recover(resumeFile)
        print("enderecos", os.listdir(os.getcwd()))
        print("pasta atual ", os.path.dirname(os.path.realpath(__file__)))
        current_dir = os.path.dirname(os.path.realpath(__file__))
        if 'attachments' not in os.listdir(current_dir):
        # if 'attachments' not in os.listdir(os.getcwd()):
            os.mkdir(current_dir+'/attachments')
        for msg in generate_mail_messages(user_name, password, resumeFile):
            save_attachments(user_name, msg, current_dir+'/attachments')
